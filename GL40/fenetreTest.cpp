...
FenetreTest::FenetreTest(int nombreClics, QWidget *parent, double parametre1, double parametre2) : QWidget(parent) {
    ...
    connect(this, SIGNAL(sequenceFin(std::vector<Statistiques>)), this->parent(), SLOT(afficheFenetreStatistiques(std::vector<Statistiques>)));

}

void FenetreTest::mouseMoveEvent(QMouseEvent *) {
...
void FenetreTest::deplacerBoutonClic() {
    ...
    if(nombreClicsCourant >= 2) {
        ...
        statistiques.push_back(Statistiques(positionInitiale, QPoint(x, y), largeurBouton, chronometre->elapsed(), param1, param2));
    }
    ...
    if(nombreClicsCourant - 1 == nombreC) {
        ...
        for(unsigned int i=0; i<statistiques.size(); i++) {
            ...
            statistiques[i].calculDistance();
            statistiques[i].calculTempsFitts();

        }

        emit sequenceFin(statistiques);
    }
