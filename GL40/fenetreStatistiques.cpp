...
FenetreStatistiques::FenetreStatistiques(vector<Statistiques> statistiques, QWidget *parent) : QWidget(parent) {
    ...
    for(unsigned int i=1; i<statistiques.size()+1; i++) {

        modeleTableau->setItem(i, 0, new QStandardItem(QString::number(statistiques[i-1].getDistance())));
        modeleTableau->setItem(i, 1, new QStandardItem(QString::number(statistiques[i-1].getLargeurBouton())));
        modeleTableau->setItem(i, 2, new QStandardItem(QString::number(statistiques[i-1].getTempsFitts())));
        modeleTableau->setItem(i, 3, new QStandardItem(QString::number((double)statistiques[i-1].getTempsDeplacementSouris()/1000)));

        // Calculate average times
        tempsTotalReel += (double)statistiques[i-1].getTempsDeplacementSouris()/1000;
        tempsTotalFitts += statistiques[i-1].getTempsFitts();
    }

    modeleTableau->setItem(statistiques.size()+1, 1, new QStandardItem("Temps moyen"));
    modeleTableau->setItem(statistiques.size()+1, 2, new QStandardItem(QString::number(tempsTotalFitts/statistiques.size())));
    modeleTableau->setItem(statistiques.size()+1, 3, new QStandardItem(QString::number(tempsTotalReel/statistiques.size())));

    // Alignement central dans les cellules
    for(unsigned int i = 0; i<statistiques.size()+2; i++) {
    ...
    // Dessin
    QChartView *chartView = new QChartView(createLineChart(statistiques), parent);
...
QPushButton* FenetreStatistiques::getBoutonRecommencer() {
    return recommencer;
}

QChart* FenetreStatistiques::createLineChart(vector<Statistiques> statistiques) const {
    ...
    for (unsigned int i=1; i<statistiques.size()+1; i++) {
        serieTheorique->append(i, statistiques[i-1].getTempsFitts());
        serieReel->append(i, (double)statistiques[i-1].getTempsDeplacementSouris()/1000);
    }
...
void FenetreStatistiques::quitterApplication() {

    parentWidget()->close();

}

void FenetreStatistiques::retournerMenu() {
